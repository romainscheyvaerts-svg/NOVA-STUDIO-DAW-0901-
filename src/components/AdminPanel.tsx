...
const AdminPanel: React.FC<AdminPanelProps> = ({ user, onSuccess, onClose, existingInstruments }) => {
  ...
  const handleDownloadCover = () => {
    if (!coverPreviewUrl || !coverPreviewUrl.startsWith('data:')) {
        setStatus("❌ Aucune cover générée par IA à télécharger.");
        return;
    }
    const link = document.createElement('a');
    link.href = coverPreviewUrl;
    const safeName = (name || 'untitled-beat').replace(/[^a-z0-9]/gi, '_').toLowerCase();
    link.download = `ai-cover-${safeName}.png`;
    ...
    try {
      ...
      if (editingId) {
          ...
          await supabaseManager.updateInstrument(editingId, {
              ...
              image_url: coverUrl, preview_url: previewUrl, stems_url: stemsUrl || undefined,
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] bg-black/95 flex justify-center items-center p-6 animate-in fade-in duration-300">
      ...
      <div className="w-full max-w-7xl h-[90vh] bg-[#14161a] border border-white/10 rounded-3xl flex flex-col overflow-hidden shadow-2xl relative">
        ...
        <div className="flex-1 flex overflow-hidden">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-4 bg-white/5 p-4 rounded-xl border border-white/5">
                            ...
                            <div className="relative">
                                <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-lg pl-3 pr-8 py-2 text-xs text-white focus:border-cyan-500 outline-none" placeholder="Nom du Beat" />
                                <button type="button" onClick={() => handleRegenerateName()} disabled={isGeneratingMeta} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 hover:text-cyan-400" title="Régénérer le nom"><i className={`fas fa-dice ${isGeneratingMeta ? 'fa-spin' : ''}`}></i></button>
                            </div>
                        ...
                        <div className="space-y-4 bg-white/5 p-4 rounded-xl border border-white/5">
                            ...
                            <div className="flex space-x-3">
                                <div className="flex flex-col items-center space-y-2">
                                    <div className="w-20 h-20 bg-black rounded-lg border border-white/10 flex items-center justify-center overflow-hidden shrink-0 relative group">
                                        ...
                                        <button type="button" onClick={handleGenerateCover} className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center text-cyan-400 transition-opacity" title="Régénérer Cover"><i className="fas fa-sync-alt"></i></button>
                                    </div>
                                    {coverPreviewUrl && coverPreviewUrl.startsWith('data:') && (
                                        <button type="button" onClick={handleDownloadCover} className="px-3 py-1.5 bg-green-600/10 text-green-400 text-[9px] font-black uppercase rounded-lg border border-green-500/20 hover:bg-green-500 hover:text-black transition-colors flex items-center space-x-2">
                                ...
                                <div className="flex-1 space-y-2">
                                    ...
                                    <label htmlFor="cover-upload" className="block w-full py-1.5 bg-white/10 hover:bg-white/20 text-center rounded-lg text-[9px] font-bold text-slate-300 cursor-pointer transition-all">{coverFile ? "Fichier Sélectionné" : "Changer l'image"}</label>
                                    <div className="flex space-x-2">
                                        ...
                                        <button type="button" onClick={handleGenerateCover} disabled={isGeneratingImg} className="px-3 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-xs" title="Générer avec ce prompt"><i className={`fas ${isGeneratingImg ? 'fa-spinner fa-spin' : 'fa-magic'}`}></i></button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <div className={`flex flex-col p-2 rounded-lg border ${previewFile ? 'bg-green-500/10 border-green-500/30' : 'bg-black/20 border-white/5'}`}>
                                    ...
                                    {localPreviewUrl && (<div className="mt-2 flex items-center bg-black/40 rounded px-2 py-1">
                                        <button type="button" onClick={() => togglePreview(localPreviewUrl!)} className={`mr-2 ${playingUrl === localPreviewUrl ? 'text-green-400' : 'text-slate-400 hover:text-white'}`}><i className={`fas ${playingUrl === localPreviewUrl ? 'fa-stop' : 'fa-play'} text-xs`}></i></button>
                                        <div className="h-1 bg-white/10 flex-1 rounded-full overflow-hidden"><div className={`h-full bg-green-500 ${playingUrl === localPreviewUrl ? 'animate-pulse' : ''}`} style={{width: '100%'}}></div></div><span className="ml-2 text-[7px] text-slate-500">PRÉ-ÉCOUTE</span>
                                    </div>)}
                                    {isConverting && (<div className="mt-1 text-[8px] text-cyan-400 animate-pulse text-center"><i className="fas fa-cog fa-spin mr-1"></i> Encodage MP3 en cours...</div>)}
                                </div>
                                <div className={`flex items-center space-x-2 p-2 rounded-lg border ${importedStemsUrl ? 'bg-green-500/10 border-green-500/30' : 'bg-black/20 border-white/5'}`}>
                                    ...
                                    <div className="flex-1 min-w-0">{importedStemsUrl ? (<span className="text-[9px] font-mono text-green-300">🔗 Fichier Drive Lié (STEMS)</span>) : (<input type="file" ref={stemsInputRef} accept=".zip,.rar" onChange={(e) => handleFileChange(e, 'stems')} className="text-[9px] text-slate-400 file:bg-white/10 file:text-white file:border-0 file:rounded-md file:px-2 file:py-0.5 file:mr-2 cursor-pointer w-full" />)}</div>
                        ...
                        <div className="pt-2">
                            ...
                            <button type="submit" disabled={loading || isConverting} className={`w-full h-12 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg transition-all disabled:opacity-50 ${editingId ? 'bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-black shadow-amber-500/20' : 'bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white shadow-cyan-500/20'}`}>{loading || isConverting ? <i className="fas fa-spinner fa-spin"></i> : (editingId ? "Mettre à jour" : "Mettre en ligne")}</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col bg-[#14161a]">
                <div className="p-6 border-b border-white/5 flex justify-between items-center"><h3 className="text-xs font-black uppercase tracking-widest text-slate-300"><i className="fas fa-list mr-2"></i>Inventaire ({inventory.length})</h3></div>
                <div className="flex-1 overflow-y-auto custom-scroll p-6">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {inventory.map((inst) => (<tr key={inst.id} className={`hover:bg-white/[0.02] transition-colors ${editingId === inst.id ? 'bg-amber-500/5' : ''}`}>
                                    <td className="py-3 text-center">
                                         <button onClick={() => togglePreview(inst.preview_url)} className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${playingUrl === supabaseManager.getPublicInstrumentUrl(inst.preview_url) ? 'bg-cyan-500 text-black shadow-lg shadow-cyan-500/30' : 'bg-white/5 text-slate-400 hover:text-white hover:bg-white/10'}`}><i className={`fas ${playingUrl === supabaseManager.getPublicInstrumentUrl(inst.preview_url) ? 'fa-pause' : 'fa-play'} text-xs`}></i></button>
                                    </td>
                                    <td className="py-3 pl-2"><img src={inst.image_url} alt="cover" className="w-10 h-10 rounded-md object-cover border border-white/10" /></td>
                                    <td className="py-3"><div className="text-xs font-bold text-white">{inst.name}</div><div className="text-[9px] text-slate-500">{inst.category}</div></td>
                                    <td className="py-3"><span className="text-xs font-mono text-green-400">${inst.price_basic}</span></td>
                                    <td className="py-3 text-center"><button onClick={() => toggleVisibility(inst.id, inst.is_visible)} className={`w-10 h-5 rounded-full relative transition-colors duration-300 ${inst.is_visible ? 'bg-green-500' : 'bg-slate-700'}`}><div className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform duration-300 ${inst.is_visible ? 'translate-x-5' : 'translate-x-0'}`} /></button></td>
                                    <td className="py-3 text-right pr-2 space-x-2">
                                        <button onClick={() => handleEditClick(inst)} className="w-8 h-8 rounded-lg bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-black transition-all inline-flex items-center justify-center" title="Modifier"><i className="fas fa-pencil-alt text-xs"></i></button>
                                        <button onClick={() => deleteInstrument(inst.id)} className="w-8 h-8 rounded-lg bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white transition-all inline-flex items-center justify-center" title="Supprimer"><i className="fas fa-trash text-xs"></i></button>
                                    </td>
                            </tr>))}
                        </tbody>
                    </table>
